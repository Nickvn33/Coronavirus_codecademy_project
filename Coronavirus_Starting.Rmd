...
total_case_northern_hemisphere <- confirmed %>%
  filter(Lat > 0) %>%
    select("3/22/20") %>%
      sum()
total_case_northern_hemisphere
...
total_cases_australia <- confirmed %>%
  filter(`Country/Region` == "Australia") %>%
    select("3/22/20") %>%
      sum()
total_cases_australia
# Display percentages
total_case_northern_hemisphere / total_cases * 100
...
countries_confirmed <- confirmed %>%
  group_by(`Country/Region`) %>%
    summarise_at(vars(-Lat, -Long, -`Province/State`), sum)
# countries_confirmed
...
countries_recovered <- recovered %>%
  group_by(`Country/Region`) %>%
    summarise_at(vars(-Lat, -Long, -`Province/State`), sum)
total_us <- countries_confirmed %>%
  filter(`Country/Region` == "US") %>%
    select("3/22/20")
recovered_us <- countries_recovered %>%
  filter(`Country/Region` == "US") %>%
    select("3/22/20")
...
confirmed_transposed <- countries_confirmed %>%
  t() %>%
    as.data.frame()
...
confirmed_transposed %>% select(US) %>% max()
```
...
# Add the date column
confirmed_transposed <- confirmed_transposed %>%
  mutate(date = c(1:nrow(confirmed_transposed)))
head(select(confirmed_transposed, date))
...
confirmed_transposed %>% ggplot(aes(x = date, y = Italy)) + geom_line()
```
...
confirmed_transposed %>% ggplot(aes(x = date, y = Italy)) + geom_line() + labs(x = "Days Since January 22nd, 2020", y = "Confirmed Cases - cumulative", title = "Confirmed COVID-19 Cases - Italy")
```
